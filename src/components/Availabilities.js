...
const Avail = (props) => {
    const { start_time, end_time, editable, id, setUpdates, updates } = props;
    const [range, setRange] = useState([new Date(start_time), new Date(end_time)]);
    useEffect(() => {
        updateAvail({ date: range, id: id });
    }, [range, id]);
    return (<>
        <DateTimeRangePicker disabled={!editable} value={range} onChange={(date) => {
            setRange(date);
